                ...
                <div class="i18n-input tab-pane fade <?= $key === 0 ? 'active show' : '' ?>" id="<?= $name . '-' . $language ?>">
                    <div class="form-group">
                        ...
                        @endif
                            ...
                            {{ Form::textarea( sprintf( '%s[%s]', $language, $name ), old( $name, ( Form::getModel() && Form::getModel()->translate($language) ) ? Form::getModel()->translate($language)->{$name} : null ), array_merge( [ 'class' => $errors->has( $errorKey ) ? 'form-control is-invalid' : 'form-control' ], $attributes ) ) }}
...
@else
    <div class="form-group">
        ...
        @endif
            ...
            {{ Form::textarea( $name, old( $name, $value ), array_merge( [ 'class' => $errors->has( $errorKey ) ? 'form-control is-invalid' : 'form-control' ], $attributes ) ) }}
